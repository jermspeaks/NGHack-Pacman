...
func (handle *Handle) cacheHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method != "GET" {
		http.Error(w, "Method not allowed", 405)
		return
	}
	http.ServeFile(w, r, "../Client/cache.manifest")
}

func (handle *Handle) fontsHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method != "GET" {
		http.Error(w, "Method not allowed", 405)
		return
	}
	...
	f := p[len(p)-1]
	http.ServeFile(w, r, "../Client/fonts/"+f)
}

func (handle *Handle) mp3Handler(w http.ResponseWriter, r *http.Request) {
	if r.Method != "GET" {
		http.Error(w, "Method not allowed", 405)
		return
	}
	p := strings.Split(r.URL.Path, "/")
	f := p[len(p)-1]
	http.ServeFile(w, r, "../Client/mp3/"+f)
}

func (handle *Handle) wavHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method != "GET" {
		http.Error(w, "Method not allowed", 405)
		return
	}
	p := strings.Split(r.URL.Path, "/")
	f := p[len(p)-1]
	http.ServeFile(w, r, "../Client/wav/"+f)
}
...
func (handle *Handle) jsHandler(w http.ResponseWriter, r *http.Request) {
	...
	p := strings.Split(r.URL.Path, "/")
	f := p[len(p)-1]
	http.ServeFile(w, r, "../Client/js/"+f)
}

func (handle *Handle) dataHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method != "GET" {
		http.Error(w, "Method not allowed", 405)
		return
	}
	p := strings.Split(r.URL.Path, "/")
	f := p[len(p)-1]
	http.ServeFile(w, r, "../Client/data/"+f)
}

func (handle *Handle) imgHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method != "GET" {
		http.Error(w, "Method not allowed", 405)
		return
	}
	p := strings.Split(r.URL.Path, "/")
	f := p[len(p)-1]
	http.ServeFile(w, r, "../Client/img/"+f)
}
...
func (handle *Handle) cssHandler(w http.ResponseWriter, r *http.Request) {
	...
	p := strings.Split(r.URL.Path, "/")
	f := p[len(p)-1]
	http.ServeFile(w, r, "../Client/css/"+f)
}
...
func (handle *Handle) rootHandler(w http.ResponseWriter, r *http.Request) {
	...
	rootTempl := template.Must(template.ParseFiles("../Client/index.htm"))
